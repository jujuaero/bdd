...
public class WorkshopController {
    ...
    private boolean showWorkshopDialog(String title, Workshop workshop, boolean editing) {
        ...
        grid.add(new Label("Max Participants:"), 0, 4); grid.add(maxField, 1, 4);
        ...
        try {
            workshop.setDurationMinutes(Integer.parseInt(durationField.getText().trim()));
            workshop.setMaxParticipants(Integer.parseInt(maxField.getText().trim()));
            workshop.setPrice(Double.parseDouble(priceField.getText().trim()));
        } catch (Exception e) {
            showError("Validation", "Duration, Max Participants and Price must be numeric.");
            return false;
        }
